...
const getData = async () => {
  const auth = new google.auth.GoogleAuth({
    keyFile:
      "/home/saisandeep845/Desktop/IP-Assignment/IP-Assignment/Backend/Credentials/wise-bongo-451704-r2-0dc4eccc1b1f.json",
...
export const stdThroughCDC = async (req, res, next) => {
  try {
    ...
    const stdThroughcdc = data.filter(
      (student) =>
        student.placementSource &&
        student.placementSource.includes("Through CDC")
    );
...
export const std_Not_ThroughCDC = async (req, res, next) => {
  try {
    ...
    const std_Not_Throughcdc = data.filter(
      (student) =>
        student.placementSource &&
        student.placementSource.includes("Off Campus")
    );
...
export const std_Industry_intern = async (req, res, next) => {
  try {
    ...
    const std_indurstry = data.filter(
      (student) =>
        student.internshipType && student.internshipType.includes("Industry")
    );
...
export const stdCompanyWise = async (req, res, next) => {
  try {
    ...
    if (!company) {
      return res
        .status(400)
        .json({ message: "Company name is required in the request body." });
    }
    const data = await getData();
  ...
  try {
    ...
    if (!period) {
      return res
        .status(400)
        .json({
          message: "Internship period is required in the request body.",
        });
    }
...
export const stdByNameRegNo = async (req, res, next) => {
  try {
    ...
    const students = await getData(); // Fetch Google Sheets data

    // Filter data based on regNumber (exact match) or name (case-insensitive match)
    const filteredStudents = students.filter((student) =>
      student.regNumber === searchQuery || student.name.toLowerCase().includes(searchQuery.toLowerCase())
    );
